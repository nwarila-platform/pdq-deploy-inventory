...
```
ansible/applications/pdq/              # the role (skeleton; built per _handoff/QUEUE.md)
...
_handoff/                              # RESTART (read first), QUEUE, READMAP, REVIEW, loop/, steps/
terraform/                            # skeleton — NOT ACTIVE (AWS PoC layer deferred)
```

## Dev process

Read `_handoff/RESTART.md` first. Work proceeds one command per cycle through the adapted
strict-cycle (`_handoff/loop/STRICT-CYCLE-adapted.md`): Claude plans (with research) → Director
consulted → Codex adversarially reviews → Codex executes → Claude validates gate + style → merge
with a `_handoff/REVIEW.md` ledger row. The build queue is `_handoff/QUEUE.md`.
...
`READMAP.md` (and the sibling IAM audit in `../windows-wsus/_handoff/AWS-IAM-AUDIT-wazuh.md`: the
wazuh IAM must be finalized before it is cloned here).
