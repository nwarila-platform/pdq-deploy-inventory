...
- **Proper fix:** framework PR "loader v3.1 Windows support" — guard `package_facts`
  ...
  **Gate:** as a loader change, this proposal must pass
  the loader-change gate (two independent reviewers,
...
- **Interim gate (used from C01 on, proposed at review and independently validated):**
  `ansible-lint --warn-list 'yaml[comments]' applications/wsus` — 0 failures /
  34 warnings, matching the yamllint stance. The verification snippet in the repo docs
...
- **What:** `ansible/applications/wsus/tasks/main.yml` was bumped **v3.0.0 → v3.1.0** under a
  one-time Director-approved exception (dual independent review panel: unanimous APPROVE,
...
- **Evidence:** the V-piece record + both reviewer verdicts (loader-change gate).
