...
<#
    Pester spec for Set-PdqInventoryRegistration.ps1 (org pair convention: every script
...
BeforeAll {
  $script:ScriptPath = Join-Path -Path $PSScriptRoot -ChildPath 'Set-PdqInventoryRegistration.ps1'
  ...
  Function New-FakeLicense {
    ...
    $Xml = '<License Version="2.0" ID="{0}" Name="PDQ Inventory" Mode="Enterprise" E-Mail="{1}" />' -f $Id, $Email
  ...
  Function Get-ItemProperty {
    ...
    If ($LiteralPath -ne 'HKLM:\SOFTWARE\Admin Arsenal\PDQ Inventory') {
  ...
  New-Item -Force -Path 'function:C:\Program Files (x86)\Admin Arsenal\PDQ Inventory\PDQInventory.exe' -Value {
  ...
  New-Item -Force -Path 'function:C:\Program Files (x86)\Admin Arsenal\PDQ Inventory\sqlite3.exe' -Value {
    $Database, $Sql = $args
...
Describe 'Set-PdqInventoryRegistration' {
  It 'declares SupportsShouldProcess so the module runs it in check mode' {
    ...
    $Script = Get-Content -Raw (Join-Path $PSScriptRoot 'Set-PdqInventoryRegistration.ps1')
