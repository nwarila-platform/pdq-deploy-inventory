...
<#
    .SYNOPSIS
        Records the PDQ Inventory registration the first console launch would otherwise ask for.
    ...
    .DESCRIPTION
        ...
        Rows are written as the popup writes them (the same schema PDQ Deploy uses, measured
        2026-08-22): LicensedMachine and LicensedUser get a fresh GUID apiece when absent
        (lower-case names, the machine and its built-in administrator), and Registration is keyed
        on all three ids with ProductMode 'Server', RegistrationStatus 'Registered' and marketing
        declined. A row that already matches leaves everything untouched; after any write the row
        is read back and must equal what was asked, so a write that did not take fails the run.
        ...
        scripts/ with the sibling Set-PdqInventoryRegistration.pester.ps1 spec; the role tracks
        only files/Set-PdqInventoryRegistration.ps1.stub, resolved by the build.
    ...
    .EXAMPLE
        .\Set-PdqInventoryRegistration.ps1 -Email 'someone@example.com'
...
# 2026-08-22), so a path parameter would advertise a choice that does not exist.
...
New-Variable -Force -Name:'LICENSE_KEY' -Option:('Private', 'ReadOnly') -Value:(
  [System.String]'HKLM:\SOFTWARE\Admin Arsenal\PDQ Inventory'
)
...
# The popup records lower-case names, measured 2026-08-22; matching its casing keeps a converged
