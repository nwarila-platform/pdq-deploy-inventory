...
<#
    Pester spec for Set-PdqInventorySetting.ps1 (org pair convention: every script ships
    ...
    section, values on a `value` attribute; the sections the real export nests
    under a ViewModel are rendered that way, so the leaf's last two components
    match the command line's name. The write stubs mutate it, so a test states
    an outcome rather than a sequence of calls. A name listed in
    $global:FakeIgnored is accepted and NOT stored, which is the product's real
    and dangerous behaviour: it reports success for names it does not use.
#>
...
BeforeAll {
  $script:ScriptPath = Join-Path -Path $PSScriptRoot -ChildPath 'Set-PdqInventorySetting.ps1'
  # The context every call carries: where the database lives, from which the script derives the two
  # beside-the-database locations. The fake export is seeded to match, so derivation reads as
  # unchanged and each case still exercises exactly the settings it names. Inventory has no
  # repository, so there is no share name to pass.
  $script:Ctx = @{
    DatabaseDrive     = 'D'
  ...
  # Renders $global:FakeSettings the way the product's export renders it. The
  # sections listed in $Nested are emitted under their ViewModel parent, so the
  # leaf's full trail is three deep and only its last two components match the
  # command line's name; the section element carries a 'name' ATTRIBUTE exactly
  # as the product writes it, which is what PowerShell's XML adapter hands back
  # in place of the tag when a walker asks for .Name -- the hazard the script's
  # LocalName read has to survive.
  Function global:Write-FakeExport {
    $Nested = @{
      'DatabaseBackupSettings' = 'DatabaseSettingsViewModel'
      'AuditLogSettings'       = 'LoggingSettingsViewModel'
      'SentrySettings'         = 'LoggingSettingsViewModel'
      'RemoteDesktopSettings'  = 'RemoteControlSettingsViewModel'
      'VncSettings'            = 'RemoteControlSettingsViewModel'
    }
    $Lines = [System.Collections.Generic.List[System.String]]::new()
    $Lines.Add('<?xml version="1.0" encoding="utf-8"?>')
    $Lines.Add('<AdminArsenal.Export Code="PDQInventory" Name="PDQ Inventory" Version="20.1.8.0">')
    ForEach ($Name In ($global:FakeSettings.Keys | Sort-Object)) {
      ...
      If ($Nested.ContainsKey($Parts[0])) {
        $Lines.Add(('  <{0}>' -f $Nested[$Parts[0]]))
        $Lines.Add(('    <{0} name="{0}">' -f $Parts[0]))
...
Describe 'Set-PdqInventorySetting' {
  It 'declares SupportsShouldProcess so the module runs it in check mode' {
    ...
    $Script = Get-Content -Raw (Join-Path $PSScriptRoot 'Set-PdqInventorySetting.ps1')
  ...
  BeforeEach {
    ...
    # A spread of the real surface: two flat sections, two nested-under-a-ViewModel sections, the
    # two beside-the-database locations seeded to match the Ctx derivation, a printing setting
    # (whose stored name differs from the export name), and an audit-log path.
    $global:FakeSettings = @{
      'ScanSettings.CleanupLogDays'             = '14'
      'PerformanceSettings.MaxServerThreads'    = '32'
      'DatabaseBackupSettings.Keep'             = '10'
      'DatabaseBackupSettings.BackupDirectory'  = 'D:\PDQ Inventory\Backups'
      'AuditLogSettings.VerboseFileDirectory'   = 'D:\PDQ Inventory\Logs'
      'AuditLogSettings.CustomConfigPath'       = ''
      'AnalyticsSettings.CollectAnalyticsUsage' = 'true'
      'PrintingSettings.IsInColor'              = 'true'
      'PrintingSettings.MarginTop'              = '20'
      'PrintingSettings.HeaderText'             = ''
    }
    $global:FakeIgnored = @()
    $global:FakeDbRows = @{}
    ...
    New-Item -Force -Path ('function:global:' + $script:CliPath) -Value {
      ...
      Switch ($args[0]) {
        ...
        'SystemInfo' {
          ...
          Return @('Database     : D:\PDQ Inventory\Database.db', 'Version      : 20.1.8.0')
        }
        'Settings' {
          ...
          # The real product stores printing under a 'Product' prefix and
          # publishes it in the export without it; the fake mirrors that.
    ...
    # The sqlite tool. The settle poll reads the Settings table; the open-console read lists
    # sessions. Nothing else reaches it -- Inventory has no system-variable settings.
    New-Item -Force -Path ('function:global:' + $script:SqlitePath) -Value {
      $global:FakeSqliteCalls.Add($args -join ' ')
      # The statement is the last SELECT argument, wherever the mode flags put it.
      $Sql = @($args | Where-Object -FilterScript { $PSItem -like 'SELECT *' })[-1]
  ...
  AfterAll {
    Remove-Variable -Name 'FakeSettings', 'FakeIgnored', 'FakeDbRows', 'FakeConsoleSessions',
      'FakeSessionFails', 'FakeDrainStalls', 'FakeCliCalls', 'FakeSqliteCalls', 'FakeExportFails',
      'FakeSystemInfoBlank', 'FakeExportPath' -Scope 'Global' -Force -ErrorAction 'SilentlyContinue'
  }
  ...
  Context 'the table as the contract' {
    ...
    It 'refuses a name it does not carry, rather than storing a row nothing reads' {
      { & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_day' = 45 } } |
    ...
    It 'refuses a value of the wrong type before anything is read or written' {
      { & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_log_days' = 'soon' } } |
    ...
    It 'treats a declared-but-null value as unmanaged' {
      $Result = & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_log_days' = $Null } | ConvertFrom-Json

      # Only the two beside-the-database locations remain, both derived.
  ...
  Context 'deciding what to write' {
    ...
    It 'writes nothing when every declared setting already matches' {
      $Result = & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_log_days' = 14 } | ConvertFrom-Json

      $Result.changed | Should -BeFalse
      $Result.unchanged | Should -Contain 'ScanSettings.CleanupLogDays'
      @($global:FakeCliCalls | Where-Object { $_ -like 'Settings*' }).Count | Should -Be 0
    }
    ...
    It 'fails loudly when the console-session read errors instead of reporting no console' {
      ...
      { & $script:ScriptPath @script:Ctx -Preference @{ scanning = @{ cleanup_log_days = 45 } } 2>$null } |
    ...
    It 'takes the console shape: pages holding their settings' {
      ...
      $Result = & $script:ScriptPath @script:Ctx -Preference @{
        scanning    = @{ cleanup_log_days = 45 }
        performance = @{ total_concurrent_targets = 32 }
      } | ConvertFrom-Json
      $Result.applied | Should -Be @('ScanSettings.CleanupLogDays')
      $Result.unchanged | Should -Contain 'PerformanceSettings.MaxServerThreads'
    }
    ...
    It 'names whoever has a console open when it writes' {
      ...
      $Result = & $script:ScriptPath @script:Ctx -Preference @{ scanning = @{ cleanup_log_days = 45 } } |
        ConvertFrom-Json
      $Result.open_consoles | Should -Be @('HOST\\someone')
      $Result.msg | Should -Match 'console is open'
      $global:FakeConsoleSessions = @()
      $Quiet = & $script:ScriptPath @script:Ctx -Preference @{ scanning = @{ cleanup_log_days = 14 } } |
        ConvertFrom-Json
    ...
    It 'restores a blank default by resetting, never by writing an empty value' {
      # The command line refuses -Set with an empty value, so blank travels as -Reset.
    ...
    It 'writes only the setting that differs, leaving the rest alone' {
      $Result = & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_log_days' = 45; 'performance.total_concurrent_targets' = 32 } |
        ConvertFrom-Json

      $Result.applied | Should -Be @('ScanSettings.CleanupLogDays')
      $Result.unchanged | Should -Contain 'PerformanceSettings.MaxServerThreads'
      $Result.changed | Should -BeTrue
      $global:FakeSettings['ScanSettings.CleanupLogDays'] | Should -Be '45'
      @($global:FakeCliCalls | Where-Object { $_ -like 'Settings*' }).Count | Should -Be 1
    }

    It 'manages only the parameters that were passed' {
      $Result = & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_log_days' = 45 } | ConvertFrom-Json

      # The one passed setting, plus the two derived beside-the-database locations.
      $Result.requested | Should -Be 3
      $global:FakeSettings['PerformanceSettings.MaxServerThreads'] | Should -Be '32'
    }
    ...
    It 'finds a setting the export nests under a view model by its two-part name' {
      ...
      # DatabaseSettingsViewModel.DatabaseBackupSettings.Keep -- while the product's command line
      # takes the last two components. The caller states neither: it passes a parameter, and the
      # script indexes both spellings so the nesting never reaches the playbook.
      $Result = & $script:ScriptPath @script:Ctx -Preference @{ 'database.backups_to_keep' = 10 } |
        ConvertFrom-Json

      $Result.unchanged | Should -Contain 'DatabaseBackupSettings.Keep'
      $Result.changed | Should -BeFalse
    }

    It 'proves an export-invisible setting against its database row' {
      # ShowCollectionItemCounts is real -- the console displays it -- yet the export never
      # publishes it; its table row carries Unexported, and compare and verify read the database
      # instead. Modeled here by an export that drops the name while the row lands.
      ...
      $Again.unchanged | Should -Contain 'InterfaceSettings.ShowCollectionItemCounts'
  ...
  Context 'proving the write landed' {
    ...
    It 'reports a setting the product accepted and discarded as ignored, and fails' {
      $global:FakeIgnored = @('ScanSettings.CleanupLogDays')

      $Output = & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_log_days' = 45 }
      $Result = $Output | ConvertFrom-Json

      $Result.ignored | Should -Be @('ScanSettings.CleanupLogDays')
    ...
    It 'exports twice: once to decide, once to prove' {
      & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_log_days' = 45 } | Out-Null
    ...
    It 'fails loudly when the export itself fails' {
      ...
      { & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_log_days' = 45 } } | Should -Throw
    }
  }

  Context 'the database the settle poll reads' {
    ...
    It 'asks the product where its database is rather than assuming' {
      ...
      # The settle poll reads the Settings table from the path SystemInfo reported; the -csv flag
      # precedes the path in the call, so the assertion is that the path is used, not that it leads.
  ...
  Context '$Ansible transport' {
    ...
    It 'sets Changed=$False explicitly when nothing differs' {
      ...
      & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_log_days' = 14 } | Out-Null
    ...
    It 'fails the task when a setting was ignored, publishing the evidence first' {
      ...
      $Context.Result.ignored | Should -Be @('ScanSettings.CleanupLogDays')
    }
    ...
    It 'reports the would-be change in check mode and writes nothing' {
      ...
      $global:FakeSettings['ScanSettings.CleanupLogDays'] | Should -Be '14'
    ...
    It 'runs in check mode under the transport -WhatIf without crashing or leaking' {
      ...
      $Context = New-AnsibleContext -CheckMode
      & $script:ScriptPath @script:Ctx -Preference @{ 'scanning.cleanup_log_days' = 45 } -WhatIf | Out-Null
      $Context.Changed | Should -BeTrue
      $global:FakeSettings['ScanSettings.CleanupLogDays'] | Should -Be '14'
      Test-Path -LiteralPath $script:ExportPath | Should -BeFalse
    }
